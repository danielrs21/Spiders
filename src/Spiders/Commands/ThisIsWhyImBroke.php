...
class ThisIsWhyImBroke extends Command
{
    /**
    ...
    */
    protected $info = [
        'site_name'     => 'ThisIsWhyImBroke',
        'site_base_url' => 'https://www.thisiswhyimbroke.com',
        'site_url'      => 'https://www.thisiswhyimbroke.com/gifts',
        'profit_percent'=> 1.15
    ...
    */
    protected $categories = [
        ...
        'gifts-for-geeks'           => 'DRS/Gifts/Gifts for Geeks',
        'home-and-office-gifts'     => 'DRS/Gifts/Home and Office',
        'gifts-for-kids'            => 'DRS/Gifts/Gifts for Kids',
        'food-and-drink-gifts'      => 'DRS/Gifts/Food and Drink',
        'wearable-gifts'            => 'DRS/Gifts/Wearables',
        'toy-gifts'                 => 'DRS/Gifts/Toys and Games'
    ...
    */
    ...
    {
        $this->setName('thisiswhyimbroke')
                ->setDescription('ThisIsWhyImBroke Products')
                ->setHelp('Scrapping products from ThisIsWhyImBroke.')
    ...
    */
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $output->writeln('<bg=green;options=bold;fg=white>[ SPIDERS - ThisIsWhyImBroke ]</>');
        ...
        foreach( $this->categories as $cat => $val) {
            ...
            do {
                ...
                if(!$page_html) {
                    ...
                    $this->log['error-page_url'] = $this->info['site_url']."/".$cat;
     ...
     */
     protected function getPageHtml($output,$j,$category ) {
        ...
        {    
            ...
            *   Se construye la URL,
            */
            // https://www.thisiswhyimbroke.com/api/gifts/wearable-gifts/popular/1/?max=&min=&country=VE
            $url = $this->info['site_base_url']."/api/gifts/".$category."/popular/".$j."/";
    ...
    */     
    ...
    protected function getProducts($page_html,$category, $output,$i,$cat) {
        ...
        foreach ($posts as $post) {
            ...
            $product['item_buy_url'] = $this->getItemBuyUrlClean($post->link);
            
            if(!$product['item_buy_url']) {
                $output->writeln('<error>Error obteniendo url producto (no pertenece amazon), se continua con siguiente producto</error>');
                $this->log['invalid-retail'][] = $post->link; 
                ...
                if (!$amazon_url){ //verificar que el producto existe, ademas verificar el stock, en caso de existir
                    $output->writeln('<error>Error obteniendo (no existe/no stock), se continua con siguiente producto</error>');
                    $this->log['error-product-url'][] = $post->link; 
            ...
            //$price = $page_url->find('*[id=priceblock_ourprice]',0)->plaintext;
            $price_retail                       = Common::getPrices( $amazon_url, '*[id=priceblock_ourprice]', 'plaintext',null,null,1);
            //verificar que los precios existan, si no existe llamar al continue
            if(!$price_retail['normal']){
                   $output->writeln('<error>Error obteniendo (seleccion), se continua con siguiente producto</error>');
                    $this->log['error-product-url'][] = $post->link; 
                    $this->count['error-product-url']++;
                    continue;  
            }
            $product['item_last_normal_price']  = $price_retail['normal'];
            $product['item_last_price']         = $price_retail['normal'];
            $product['item_title']              = Common::cleanString($post->title,1);
  
            $product['item_description']        = Common::cleanString($post->content,1);
            $product['item_image_url']          = $post->image;
    ...
    protected function getItemBuyUrlClean($url) {
            ...
            $validname = explode('.', $url_exploded[2]);
     
            if ($validname[1] == 'amazon') { // si la url pertenece amazon
                $url_exploded_tag = explode('&', $url_exploded[5]);
                $url_exploded_tag[0] = '?tag=drsspider-20'; //tag ID SHIFFTY
                $url_exploded_tag_ready = implode('&',$url_exploded_tag); // unir arreglo con el tag sustituto
                $url_exploded[5] = $url_exploded_tag_ready; //sustituir el tag de la url oroginal
                return implode("/", $url_exploded); //re armar la url
    ...
    protected function printLog( $output ) {
        ...
        $recordLog = Log::create('ThisIsWhyImBroke');
        $recordLog->info('++++++ SPIDERS. REPORT FOR SCRAPPING: ThisIsWhyImBroke '.date('Y-m-d H:i:s').' ++++++');
        ...
        $recordLog->info('++++++ END REPORT FOR SCRAPPING: ThisIsWhyImBroke '.date('Y-m-d H:i:s').' ++++++');
